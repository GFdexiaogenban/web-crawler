...
public class HouseInfoServiceImpl implements HouseInfoService {
    ...
    public void save(HouseTrendInfo houseTrendInfo) {
        ...
        HouseTrendInfo param = new HouseTrendInfo();
        param.setTrendUrl(houseTrendInfo.getTrendUrl());
        param.setHouseName(houseTrendInfo.getHouseName());
        param.setTrendTitle(houseTrendInfo.getTrendTitle());

        // 执行查询
        List<HouseTrendInfo> list = this.findHouseInfo(param);
        System.out.println("执行查询");
        // 判断查询结果是否为空
        if (list.size() == 0) {
            // 如果查询结果为空, 表示招聘信息数据不存在, 或者已经更新了, 需要增或者更新数据库
            this.houseInfoDao.saveAndFlush(houseTrendInfo);
        }
    ...
    public List<HouseTrendInfo> findHouseInfo(HouseTrendInfo houseTrendInfo) {
        ...
        // 设置查询条件
        Example example = Example.of(houseTrendInfo);
