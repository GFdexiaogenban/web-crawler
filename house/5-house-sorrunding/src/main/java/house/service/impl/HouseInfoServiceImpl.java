...
public class HouseInfoServiceImpl implements HouseInfoService {
    ...
    public void save(HousePhoto housePhoto) {
        ...
        HousePhoto param = new HousePhoto();
        param.setUrl(housePhoto.getUrl());
        param.setHouseName(housePhoto.getHouseName());
        param.setPicName(housePhoto.getPicName());


        // 执行查询
        List<HousePhoto> list = this.findHouseInfo(param);
        System.out.println("执行查询");
        // 判断查询结果是否为空
        if (list.size() == 0) {
            // 如果查询结果为空, 表示招聘信息数据不存在, 或者已经更新了, 需要增或者更新数据库
            this.houseInfoDao.saveAndFlush(housePhoto);
        }
    ...
    public List<HousePhoto> findHouseInfo(HousePhoto housePhoto) {
        ...
        // 设置查询条件
        Example example = Example.of(housePhoto);
